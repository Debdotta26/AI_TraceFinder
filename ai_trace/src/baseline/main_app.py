...
CSV_PATH = "C:\\Users\\ariji\\Desktop\\ai_trace\\process_data\\metadata_features.csv"
...
menu = st.sidebar.radio("Choose Action", ["Feature Explorer", "Train Models", "Evaluate Models", "Predict Scanner"])
...
if menu == "Feature Explorer":
    ...
    if uploaded_file is not None:
        ...
        try:
            pred, prob = predict_scanner(temp_path, model_choice="rf" if model_choice == "Random Forest" else "svm")
            st.success(f"🖼 Predicted Scanner: {pred}")
            st.write("🔍 Class Probabilities:")
            for cls, p in zip(joblib.load(f"models/{'random_forest' if model_choice == 'Random Forest' else 'svm'}.pkl").classes_, prob):
                st.write(f"{cls}: {p:.4f}")
